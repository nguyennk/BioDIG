...
class Application(WebServiceApplicationBase):
	def doProcessRender(self, request, key):
		renderObj = WebServiceObject()
		try:
			if request.method == "GET":
				renderObj = API.getTagGroup(request, key)
			elif request.method == "PUT":
				renderObj = API.updateTagGroup(request, key)
			elif request.method == "DELETE":
				renderObj = API.deleteTagGroup(request, key)
			else:
...
def renderAction(request, key):
	return Application().render(request, key)
